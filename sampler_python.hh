...
//! \file
//! \brief Defines the overall sampler_python object.
//!
//! This file defines the smc::sampler_python class which is used to implement entire particle systems.

#ifndef __SMC_sampler_python_HH

#define __SMC_sampler_python_HH 1.0
...
namespace smc {
  ...
  template <class Space> 
    ...
    public:
      ...
      sampler_python(long lSize, HistoryType htHistoryMode);
      ///Create an particle system constaining lSize uninitialised particles with the specified mode and random number generator.
      sampler_python(long lSize, HistoryType htHistoryMode, const gsl_rng_type* rngType, unsigned long nSeed);
      ///Dispose of a sampler_python.
      ~sampler_python();
      ...
      ///Initialise the sampler_python and its constituent particles.
      ...
      void SetMoveSet(moveset_python& pNewMoveset) { Moves = pNewMoveset; }
      ...
      ///Allow a human readable version of the sampler_python configuration to be produced using the stream operator.
      /// std::ostream & operator<< (std::ostream& os, sampler_python<Space> & s);

    private:
      ///Duplication of smc::sampler_python is not currently permitted.
	sampler_python(const sampler_python<Space> & sFrom);
      ///Duplication of smc::sampler_python is not currently permitted.
	sampler_python<Space> & operator=(const sampler_python<Space> & sFrom);
    };


  /// The constructor prepares a sampler_python for use but does not assign any moves to the moveset, initialise the particles
  ...
  template <class Space>
  sampler_python<Space>::sampler_python(long lSize, HistoryType htHM)
  {
    pRng = new rng();
    N = lSize;
  ...
  /// \tparam Space The class used to represent a point in the sample space.
  template <class Space>
  sampler_python<Space>::sampler_python(long lSize, HistoryType htHM, const gsl_rng_type* rngType, unsigned long rngSeed)
  {
  ...
  template <class Space>
  sampler_python<Space>::~sampler_python()
  {
       delete pRng;
  ...
  template <class Space>
  double sampler_python<Space>::GetESS(void) const
  {
    long double sum = 0;
    long double sumsq = 0;
  ...
  template <class Space>
  void sampler_python<Space>::Initialise(void)
  {
    T = 0;
  ...
  template <class Space>
  double sampler_python<Space>::Integrate(double(*pIntegrand)(const Space&,void*), void * pAuxiliary)
  {
  ...
  template <class Space>
  double sampler_python<Space>::IntegratePathSampling(double (*pIntegrand)(long,const particle<Space> &,void*), double (*pWidth)(long,void*), void* pAuxiliary)
  {
    if(htHistoryMode == SMC_HISTORY_NONE)
  ...
  template <class Space>
  void sampler_python<Space>::Iterate(void)
  {
    IterateEss();
    return;
  }
  ...
  template <class Space>
  void sampler_python<Space>::IterateBack(void)
  {
    if(htHistoryMode == SMC_HISTORY_NONE)
  ...
  template <class Space>
  double sampler_python<Space>::IterateEss(void)
  {
  ...
  template <class Space>
  void sampler_python<Space>::IterateUntil(long lTerminate)
  {
    while(T < lTerminate)
      Iterate();
  }
  ...
  template <class Space>
  void sampler_python<Space>::Resample(ResampleType lMode)
  {
  ...
  template <class Space>
  void sampler_python<Space>::SetResampleParams(ResampleType rtMode, double dThreshold)
  {
  ...
  template <class Space>
  std::ostream & sampler_python<Space>::StreamParticle(std::ostream & os, long n)
  {
 ...
 template <class Space>
  std::ostream & sampler_python<Space>::StreamParticles(std::ostream & os)
  {
    for(int i = 0; i < N - 1; i++)
...
namespace std {
  /// Produce a human-readable display of the state of an smc::sampler_python class using the stream operator.
  
  /// \param os The output stream to which the display should be made.
  /// \param s  The sampler_python which is to be displayed.
  template <class Space>
  std::ostream & operator<< (std::ostream & os, smc::sampler_python<Space> & s)
  {
    os << "sampler_python Configuration:" << std::endl;
    os << "======================" << std::endl;
